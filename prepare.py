...
def g2p_job(fn, txt):
    phs = [p.replace(" ", "|") for p in h2j(g2p(txt))]
    phs_str = " ".join(phs)
    os.makedirs(f'{basedir}/mfa_input', exist_ok=True)
    with open(f'{basedir}/mfa_input/{fn}.ph', 'w') as f_txt:
        phs_str = re.sub("([\!\'\,\-\.\?])+", r"\1", phs_str)
        f_txt.write(phs_str)
    with open(f'{basedir}/mfa_input/{fn}.lab', 'w') as f_txt:
        phs_str_mfa = re.sub("[\!\'\,\-\.\?]+", "PUNC", phs_str)
        phs_str_mfa = re.sub("\|", "SEP", phs_str_mfa)
        f_txt.write(phs_str_mfa)
    subprocess.check_call(f'cp "{basedir}/wavs/{fn}.wav" "{basedir}/mfa_input/"', shell=True)
...
if __name__ == "__main__":
    ...
    for l in open(f'{basedir}/metadata.csv').readlines():
        fn, _, txt = l.strip().split("|")
        futures.append(p.apply_async(g2p_job, args=[fn, txt]))
    p.close()
    mfa_dict = set()
    phone_set = set()
    ...
    meta_ori_df = pd.read_csv(os.path.join(basedir, 'metadata.csv'), delimiter='|', names=['wav', 'txt1', 'txt2'], dtype={'wav':str, 'txt1':str, 'txt2':str})
